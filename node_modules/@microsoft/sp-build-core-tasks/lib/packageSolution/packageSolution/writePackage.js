...
function writeFeature(feature, config, zip) {
    return Promise.all([writeODCFile(feature, config, zip), writeXmlFile(feature.config, config, zip)]
        ...
        if (node_core_library_1.FileSystem.getStatistics(filepath).isFile()) {
            return Promise.all([
                async () => {
                    ...
                    await writeFileToPath(icon.path, data, config, zip);
                },
...
function writeFileToPath(filename, contents, config, zip) {
    ...
    return utilities_1.serial([() => writeFile(filepath, contents), () => addFileToZip(filename, contents, zip)]);
}
/**
 * Wraps fsx.writeFile in a promise
 */
async function writeFile(filename, contents, encoding) {
